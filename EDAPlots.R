...
data <- setDT(data)[, paste0("WTIDIFF1MLag", lag_nr) := shift(WTIDIFF1M, lag_nr)][]
data <- setDT(data)[, paste0("IR24Lag", lag_nr) := shift(IR24, lag_nr)][]
data <- data[13:694,]
...
IR24Dist <- ggplot(data, aes(x = IR24Lag12)) + geom_density(fill = "#4CA3DD", col = "white") + 
  theme_classic() + theme(text = element_text(family = "Crimson", size = 15)) + theme(panel.spacing = unit(2, "lines")) + xlab("Level der Immediate Rates") + ylab("Prozent der Beobachtungen") +
  scale_y_continuous(labels = scales::percent) + facet_grid(Indicator~.)
IR24Dist
...
WTIDist <- ggplot(data, aes(x = WTIDIFF1MLag12)) + geom_density(fill = "#4CA3DD", col = "white") + 
