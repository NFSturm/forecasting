...
Der Datensatz weist eine erhebliche Disbalance zwischen in der Zahl von Rezessionen gegenüber rezessionsfreien Zeiten auf.

```{r}
ggplot(data = spread_data) + 
    geom_bar(mapping = aes(x = Indicator, y = ..prop.., group = 1), stat = "count", fill = "#4CA3DD") + 
    scale_y_continuous(labels = scales::percent_format(scale = 100)) + theme_classic() + xlab("Recession Indicator") + ylab("Proportion") + theme(text = element_text(family = "Crimson", size = 15)) + labs(title = "Klassenverhältnis des Datensatzes", subtitle = "Bezogen auf Gesamtdatensatz")
```
...
Wir verwenden einen Trainings- und einen Testing-Datensatz, entfernen zusätzlich die Datumsspalte. 
...
Für die Trainingsdaten verwenden wir die folgenden Modelle:

* Random Forest
* Boosting
* Logistische Regression
* Support Vector Machine (mit linearem und radial-basischem)
...
recall_boost <- boost_cm$byClass[1]
...
recall_svm_lin <- svm_lin_cm$byClass[1]
...
recall_svm_radial <- svm_radial_cm$byClass[1]
